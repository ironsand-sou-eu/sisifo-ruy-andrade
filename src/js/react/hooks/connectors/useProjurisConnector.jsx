...
export default function useProjurisConnector() {
  ...
  const endPoints = {
    ...
    criarTarefa: "/tarefa",
    criarLancamentoFinanceiro: (codigoProcesso) =>
      `/processo/${codigoProcesso}/lancamento-financeiro`,
  };
  ...
  async function getProjurisAuthTokenWithinExpiration() {
    ...
    if (tokenResponseIsEmpty === true)
      return await fetchAndStoreNewProjurisAuthToken();
    const tokenExpired =
      tokenResponse.projurisExpiration < new Date().getTime();
  ...
  async function fetchProjurisInfo(endPoint) {
    const uri =
      `https://api.projurisadv.com.br/adv-service/${endPoint}`.replaceAll(
        `/${endPoint}`,
        `${endPoint}`,
      );
  ...
  async function extractOptionsArray(projurisFetchResponse) {
    ...
    if (
      jsonResp.consultaTipoRetorno &&
      jsonResp.consultaTipoRetorno[0].simpleDto
    )
    ...
    if (jsonResp.andamentoTipoConsultaWs)
      return jsonResp.andamentoTipoConsultaWs;
    ...
    if (jsonResp.contaConsultaResultadoWs)
      return jsonResp.contaConsultaResultadoWs;
    return;
  }
  ...
  async function getGtCrew(
    ...
    const fetchedValues = await fetchGoogleSheetRowsMatchingExpression(
      "gts",
      grupoDeTrabalhoName,
      token,
    );
    ...
    const namesArrays = [
      ...
      estagiariosNames,
      controladoriaNames,
    ].map((commaSeparatedNames) =>
      commaSeparatedNames.split(",").map((name) => name.trim()),
    );
    const [coordenadores, advs, estagiarios, controladoria] = namesArrays.map(
      (nameArray) => getGtEntities(nameArray, allResponsaveisList),
    );
    return { coordenadores, advs, estagiarios, controladoria };
  }

  function getGtEntities(
    names,
    allResponsaveisList,
    filterTemplate = Drafter.filterTemplate,
  ) {
    if (!Array.isArray(names)) names = [names];
    const onlyOneEmptyString =
      names.length === 1 && (names[0] == "" || names[0] == undefined);
    if (names.length === 0 || onlyOneEmptyString) return undefined;
    return names.map((name) => {
      const filteredOptions = filterProjurisOptions(allResponsaveisList, {
        ...filterTemplate,
        val: name,
      });
