...
export default function useGoogleSheets() {
  ...
  async function fetchGoogleSheetRowsMatchingExpression(
    sheetName,
    expressionToSearch,
    token,
    getMany,
  ...
  function getMatchingEntry(
    ...
    const resultArray = dictionaryArray.filter(
      (pairItem) => pairItem[0].toLowerCase() === nameToFind.toLowerCase(),
    );
    if (resultArray.length >= 1) {
      ...
      return {
        found: false,
        value: errorFunction[errorKind](missingEntry, entryType),
      };
    }
  }
  ...
  return {
    fetchGoogleToken,
    fetchGoogleSheetRowsMatchingExpression,
