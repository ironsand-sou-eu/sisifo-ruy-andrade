...
export default function useProjurisCreator(msgSetter) {
  ...
  async function createAll({
    ...
    await checkIfProcessoAlreadyExists(
      projurisProcesso.processoNumeroWs[0].numeroDoProcesso,
    );
    ...
    await adjustAndAttachFaturamentosToProcesso(
      projurisFaturamentos,
      envolvidos,
    );
  ...
  async function checkIfProcessoAlreadyExists(numeroDoProcesso) {
    msgSetter.setSingleProcessingMsg(
      "Verificando se o processo já está cadastrado...",
    );
    const response = await fetchProjurisInfo(
      endPoints.buscarProcessoPorNumero + numeroDoProcesso,
    );
    const result = await extractOptionsArray(response);
    if (result.length > 0)
      throw new ProcessoAlreadyExistsException(
        codigoProcessoIfExists,
        numeroDoProcesso,
        msgSetter,
      );
  }
  ...
  async function ensurePeopleExists(partes) {
    const partesWithoutDuplicates = partes
      .uniqueValuesByParteName()
      .uniqueValuesByCpfCnpj();
    const qtdEnvolvidos = partesWithoutDuplicates.length;
    msgSetter.setSingleProcessingMsg(
      `Buscando/criando pessoas: 0 de ${qtdEnvolvidos}`,
    );
    const findPeopleResponsePromises = partesWithoutDuplicates.map(
      async (person) => {
        let body;
        if (person.cpfCnpj)
          body = JSON.stringify({ cpfOuCnpj: person.cpfCnpj });
        else body = JSON.stringify({ nomeOuRazaoSocial: person.nomePessoa });
        return await makeProjurisPost(endPoints.buscarPessoa, body);
      },
    );
    const findAllPeopleResponses = await Promise.all(
      findPeopleResponsePromises,
    );
    const findAllPeopleMatches = await Promise.all(
      findAllPeopleResponses.map(
        async (response) => await extractOptionsArray(response),
      ),
    );
    ...
    msgSetter.addMsg({
      type: "success",
      msg: `Já estavam cadastradas ${foundPersons} pessoas`,
    });
    msgSetter.addMsg({
      type: "success",
      msg: `Cadastradas ${createdPersons} pessoas novas`,
    });
  ...
  Array.prototype.uniqueValuesByParteName = function () {
    const temp = {};
    return this.filter(
      (parte) => !(temp[parte.nomePessoa] = parte.nomePessoa in temp),
    );
  };
  ...
  Array.prototype.uniqueValuesByCpfCnpj = function () {
    const temp = {};
    return this.filter(
      (parte) => !(temp[parte.cpfCnpj] = parte.cpfCnpj in temp),
    );
  };

  async function createPessoa(pessoa) {
    const projurisPessoa = new ProjurisParteDataStructure(
      pessoa.nomePessoa,
      pessoa.cpfCnpj,
      pessoa.flagSemCpfCnpj,
      pessoa.justificativaSemCpfCnpj,
      pessoa.tipoPessoa,
      pessoa.observacaoGeral,
      pessoa.telefone,
      pessoa.email,
      pessoa.habilitado,
      pessoa.profissao,
    ...
    if (!(response.status && response.status >= 200 && response.status < 300)) {
      msgSetter.addMsg({
        type: "fail",
        msg: `Erro ao criar a pessoa ${pessoa.nomePessoa}. Por favor, faça a criação manualmente.`,
      });
      return null;
    }
  ...
  async function createProcesso(processo) {
    ...
    if (!(requestSuccessful && createdSuccessful)) {
      msgSetter.addMsg({
        type: "fail",
        msg:
          "Erro ao criar o processo. Por favor, " +
    ...
    const msg =
      `Processo <a href=https://app.projurisadv.com.br/casos/` +
  ...
  async function attachPessoasToProcesso(envolvidos) {
    const names = envolvidos.map(
      (envolvido) => envolvido.pessoaEnvolvido.valor,
    );
    const params = {
      ...
      msgs: {
        ...
        failEnd:
          "como parte no processo. Por favor, faça a vinculação manualmente.",
      },
    };
  ...
  async function adjustAndAttachTarefasToProcesso(tarefas) {
    tarefas.forEach(
      (tarefa) => (tarefa.modulos[0].codigoRegistroVinculo = codigoProcesso),
    );
    const params = {
  ...
  async function adjustAndAttachAndamentosToProcesso(andamentos) {
    andamentos.forEach(
      (andamento) =>
        (andamento.modulos[0].codigoRegistroVinculo = codigoProcesso),
    );
    const params = {
      ...
      getName: (andamento) => {
        ...
        const andamentoText = subsString.substr(
          0,
          Math.min(subsString.length, subsString.lastIndexOf(" ")),
        );
        const andamentoDate = new Date(
          andamento.dataAndamento,
        ).toLocaleDateString();
  ...
  async function adjustAndAttachFaturamentosToProcesso(
    faturamentos,
    envolvidos,
  ) {
    ...
    faturamentos.forEach((faturamento) => {
      ...
      faturamento.solicitacaoRecebimentoPagtoWs.forEach(
        (solicitacao) => (solicitacao.pessoa = client),
      );
    });
    const params = {
  ...
  async function attachEntitiesToProcesso(creationParams) {
    const { entitiesArray, endpoint, checkSuccessfulCreation, getName, msgs } =
      creationParams;
