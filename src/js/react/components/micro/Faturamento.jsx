...
export default function Faturamento({
  ...
  function updateDescription({ target }) {
    const newReceitaDespesaItem = [
      { ...faturamentoItem.receitaDespesaItemWs[0], descricao: target.value },
    ];
    onChange(
      getUpdateParams({
        updatedField: "receitaDespesaItemWs",
        newValue: newReceitaDespesaItem,
      }),
    );
  }
  ...
  function validateValueAndUpdate({ target: { value } }) {
    ...
    const newReceitaDespesaItem = [
      { ...faturamentoItem.receitaDespesaItemWs[0], valor: value },
    ];
    const newPagamentoTransferencia = [
      { ...faturamentoItem.pagamentoTransferenciaWs[0], valorPagamento: value },
    ];
    const newSolicitacaoRecebimentoPagto = [
      {
        ...faturamentoItem.solicitacaoRecebimentoPagtoWs[0],
        valorSolicitacao: value,
      },
    ];
    onChange(
      getUpdateParams([
        ...
        {
          updatedField: "pagamentoTransferenciaWs",
          newValue: newPagamentoTransferencia,
        },
        {
          updatedField: "solicitacaoRecebimentoPagtoWs",
          newValue: newSolicitacaoRecebimentoPagto,
        },
  ...
  function validateDueDateAndUpdate({ target: { value } }) {
    const newPagamentoTransferencia = [
      { ...faturamentoItem.pagamentoTransferenciaWs[0], dataVencimento: value },
    ];
    const newSolicitacaoRecebimentoPagto = [
      {
        ...faturamentoItem.solicitacaoRecebimentoPagtoWs[0],
  ...
  function validateBancoAndUpdate(value) {
    ...
    const newPagamentoTransferencia = [
      { ...faturamentoItem.pagamentoTransferenciaWs[0], conta: banco },
    ];
    onChange(
      getUpdateParams({
        updatedField: "pagamentoTransferenciaWs",
    ...
    <div className="input-group d-flex" style={magicNumberStyling}>
      ...
      <div className="col-sm-2 no-padding">
        <Select
          ...
          value={insertValueLabel(
            faturamentoItem?.pagamentoTransferenciaWs[0].conta,
          )}
      ...
      <div className="no-padding">
        <Trash onClick={() => onChange(getDeleteParams())} />
      </div>
    </div>
  );
}
