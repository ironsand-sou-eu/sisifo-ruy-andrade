...
static #loadPageCheckpoints() {
    this.#divPoloAtivoTbody = document.querySelector('#poloAtivo > table > tbody')
    this.#divPoloPassivoTbody = document.querySelector('#poloPassivo > table > tbody')
}
...
static #getPartes(parteTypeToScrappe) {
    switch(parteTypeToScrappe) {
        case tiposParte.requerente:
            return this.#getCompletePartes(this.#divPoloAtivoTbody, tiposParte.requerente)
        case tiposParte.requerido:
            return this.#getCompletePartes(this.#divPoloPassivoTbody, tiposParte.requerido)
        case "outros":
            return []
    }
}
...
static #getCompletePartes(partesWrapperTbody, tipoDeParte) {
    const partes = this.#getPartesWithoutEnderecos(partesWrapperTbody, tipoDeParte)
    return partes
}
...
static #getPartesWithoutEnderecos(tbody, tipoDeParte) {
    ...
    tbody.querySelectorAll(':scope > tr').forEach(tr => {
        ...
        const parteString = tr.querySelector('td:nth-child(1) > span:nth-child(1) > span').textContent.trim()
...
static #getNameFromPje1gTjbaParteString(parteString) {
    const stringWithoutParteType = parteString.replace(/ \([A-Za-z]*\)$/, "").trim()
    const nameWithoutCpfCnpj = stringWithoutParteType
        .replace(/(CPF: \d{3}\x2E\d{3}\x2E\d{3}\x2D\d{2})|(CNPJ: \d{2}.\d{3}.\d{3}\/\d{4}-\d{2})$/, "")
        .replace(/ - $/, "")
...
static #getCpfCnpjFromPje1gTjbaParteString(parteString) {
    const cpfCnpj = parteString.match(/(\d{3}\x2E\d{3}\x2E\d{3}\x2D\d{2})|(\d{2}.\d{3}.\d{3}\/\d{4}-\d{2})/)
...
static #pushAdvogadosToParte(parteWrapperTr, parte) {
    ...
    Array.from(advogadosLis).forEach(li => {
        const advString = li.querySelector(':scope > small > span > span').textContent.trim()
