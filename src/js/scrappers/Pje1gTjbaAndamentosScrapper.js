...
static #loadPageCheckpoints() {
    this.#divTimeline = document.getElementById("divTimeLine:eventosTimeLineElement")
}
...
static async #getAndamentos() {
    ...
    for (const mediaDiv of andamentosMediaDivs) {
        const andamentoPjeType = Array.from(mediaDiv.classList).includes("tipo-D") ? "documento" : "movimentação"
        const bodyBoxDiv = mediaDiv.querySelector(".media-body")
        const horaAndamento = bodyBoxDiv.querySelector("small.text-muted").textContent.trim()
        let docName
        let docContent
        const andamento = new AndamentoDataStructure()
        if (andamentoPjeType === "documento") {
            [ andamento.id, andamento.cancelado, docName, docContent ] = await this.#getDocumentInfo(bodyBoxDiv)
        }
...
static async #getDocumentInfo(bodyBoxDiv) {
    ...
    if (isCancelado) {
        mainDocumentNameString = bodyBoxDiv.querySelector('.anexos > .anexos-inativos > span').textContent.trim()
    } else {
        const mainDocumentA = bodyBoxDiv.querySelector('.anexos > a:first-child')
        mainDocumentNameString = mainDocumentA.querySelector(":scope > span").textContent.trim()
...
static #getDate(mediaDiv, horaAndamento) {
    ...
    return Pje1gTjbaProcessoScrapper.getDateFromPjeTjbaDateString(dateString, horaAndamento)
}
...
static #getNome(bodyBoxDiv, filename = "") {
    if (filename !== "") {
        ...
        return matchParenthesisResults ? matchParenthesisResults[0].trim() : filename
    }
...
static async #waitPageLoad(iframeDocSelector) {
    ...
    while (!readystateComplete || bodyLengthChanged) {
        ...
        bodyLengthChanged = bodyLength !== iframe?.contentDocument?.body.children.length
