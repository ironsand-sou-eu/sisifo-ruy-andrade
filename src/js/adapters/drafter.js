...
import {
  tiposParte,
  projurisTipoEnvolvidoType,
} from "../utils/enumsAndHardcoded";
...
export default class Drafter {
  ...
  constructor(processoInfo, googleToken) {
    ...
    } = useProjurisConnector();
    this.#projurisHandler = {
      fetchProjurisInfo,
      extractOptionsArray,
      endPoints,
      filterProjurisOptions,
  ...
  async draftProcessoInfo() {
    ...
    if (hasErrors([this.#processoInfo]))
    ...
    const projurisProcesso = await this.#getAdaptedProcesso(
      projurisPartes.clientRole[0],
    );
    const projurisAndamentos = await this.#getAdaptedAndamentos();
    const projurisPedidos = await this.#getAdaptedPedidos(
      projurisPartes.clientRole[0]?.clientName,
      projurisProcesso.dataDistribuicao,
    );
    const projurisFaturamentos = await this.#getAdaptedFaturamentos(
      projurisPartes.clientRole[0]?.clientName,
    );
  ...
  async #loadLists() {
    const { fetchGoogleSheetData, extractValuesFromSheetsPromise } =
      useGoogleSheets();
    ...
    const projurisFetchPromises = [
      this.#projurisHandler.fetchProjurisInfo(
        this.#projurisHandler.endPoints.tiposParticipacao,
      ),
      this.#projurisHandler.fetchProjurisInfo(
        this.#projurisHandler.endPoints.varas,
      ),
      this.#projurisHandler.fetchProjurisInfo(
        this.#projurisHandler.endPoints.tiposVara,
      ),
      this.#projurisHandler.fetchProjurisInfo(
        this.#projurisHandler.endPoints.tiposJustica,
      ),
      this.#projurisHandler.fetchProjurisInfo(
        this.#projurisHandler.endPoints.areas,
      ),
      this.#projurisHandler.fetchProjurisInfo(
        this.#projurisHandler.endPoints.fases,
      ),
      this.#projurisHandler.fetchProjurisInfo(
        this.#projurisHandler.endPoints.gruposTrabalho,
      ),
      this.#projurisHandler.fetchProjurisInfo(
        this.#projurisHandler.endPoints.camposDinamicos,
      ),
      this.#projurisHandler.fetchProjurisInfo(
        this.#projurisHandler.endPoints.responsaveis,
      ),
      this.#projurisHandler.fetchProjurisInfo(
        this.#projurisHandler.endPoints.tiposTarefa,
      ),
      this.#projurisHandler.fetchProjurisInfo(
        this.#projurisHandler.endPoints.bancos,
      ),
    ];
    ...
    const [clientsList, allClientsProvisionsList, allClientsFaturamentosList] =
      await Promise.all(
        googleSheetResponses.map((response) =>
          extractValuesFromSheetsPromise(response),
        ),
      );
    const [
      tiposParticipacaoList,
      varasList,
      tiposVaraList,
      tiposJusticaList,
      areasList,
      ...
      projurisResponses.map((response) =>
        this.#projurisHandler.extractOptionsArray(response),
      ),
    );

    this.#sheetsLists = {
      clientsList,
      allClientsProvisionsList,
      allClientsFaturamentosList,
    };
    const bancosFormattedList = bancosList.map((banco) =>
      this.#formatBancoToProjuris(banco),
    );
    this.#projurisLists = {
      tiposParticipacaoList,
      varasList,
  ...
  async #getAdaptedPartes() {
    ...
    this.#pushAdaptedPartesIntoPolo(
      ...
      projurisPartes.partesRequerentes,
    );
    this.#pushAdaptedPartesIntoPolo(
      this.#processoInfo.partesRequeridas,
      projurisPartes.partesRequeridas,
    );
    this.#pushAdaptedPartesIntoPolo(
      this.#processoInfo.outrosParticipantes,
      projurisPartes.terceiros,
    );
    projurisPartes.magistrado = this.#getProjurisAdaptedMagistrado(
      this.#processoInfo.juizAtual,
    );
  ...
  #pushAdaptedPartesIntoPolo(partesArray, polo) {
    ...
    partesArray.forEach(async (parte, index) => {
      ...
      parte.advogados.forEach(async (adv, index) => {
        //TODO: Quando eu fizer a busca aninhada no Json que retorna, cadastrar os advogados como advogado adverso.
  ...
  #adaptParteToProjuris(parte, index) {
    ...
    if (parte.tipoDeParte === tiposParte.advogado) {
      projurisParte.classificacao = [
        { codigoClassificacao: 1, descricao: "Advogado" },
      ];
    ...
    if (parte.tipoDeParte === tiposParte.advogado)
      projurisParte.profissao = { chave: 616, valor: "Advogado" };
    projurisParte.tipoEnvolvido = projurisTipoEnvolvidoType[parte.tipoDeParte];
    projurisParte.tipoParticipacao =
      this.#projurisHandler.filterProjurisOptions(
        this.#projurisLists.tiposParticipacaoList,
        {
          ...Drafter.filterTemplate,
          val: parte.tipoDeParte,
          flattenOptions: true,
        },
      )[0];
  ...
  #getProjurisAdaptedMagistrado(name) {
    ...
    projurisParte.justificativaSemCpfCnpj =
      "Perfis de magistrado não têm CPF disponibilizado no Projudi";
  ...
  #identifyClientes(partes) {
    ...
    allPartes.forEach((parte) => {
      const clientParte = this.#sheetsLists.clientsList.find((client) => {
        return (
          client[0].toLowerCase().trim() ===
          parte.nomePessoa.toLowerCase().trim()
        );
      });
      if (clientParte) {
        foundClients.push({
  ...
  async #getAdaptedProcesso(clientRole) {
    ...
    const getProjurisItem = (itemType, list, filter) => {
      const matchingItems = this.#projurisHandler.filterProjurisOptions(
        list,
        filter,
      );
      if (!matchingItems) {
    ...
    const codTipoJustica = this.#getCodigoTipoJustica(
      this.#processoInfo.numero,
    );
    ...
    if (!googleJuizoInfo.found) {
      projurisProcesso.errorMsgs.push(
    ...
    const dependentFetchesResponses = await Promise.all(
      this.#makeProjurisProcessoFetches(codTipoJustica, juizoInfo.comarca),
    );
    const [instanciasCnjList, orgaosJudiciaisList] = await Promise.all(
      dependentFetchesResponses.map((response) =>
        this.#projurisHandler.extractOptionsArray(response),
      ),
    );
    projurisProcesso.vara = getProjurisItem(
      "vara",
      this.#projurisLists.varasList,
      { ...Drafter.filterTemplate, val: juizoInfo.vara },
    );
    projurisProcesso.tipoVara = getProjurisItem(
      "tipoVara",
      this.#projurisLists.tiposVaraList,
      { ...Drafter.filterTemplate, val: juizoInfo.tipoVara },
    );
    projurisProcesso.tipoJustica = getProjurisItem(
      "tipoJustica",
      this.#projurisLists.tiposJusticaList,
      {
        key: "chave",
        operator: operators.insensitiveStrictEquality,
        val: codTipoJustica,
    ...
    projurisProcesso.area = getProjurisItem(
      "area",
      this.#projurisLists.areasList,
      { ...Drafter.filterTemplate, val: "CONSUMIDOR" },
    );
    projurisProcesso.fase = getProjurisItem(
      "fase",
      this.#projurisLists.fasesList,
      { ...Drafter.filterTemplate, val: "Inicial" },
    );
    projurisProcesso.instanciaCnj = getProjurisItem(
      "instanciaCnj",
      instanciasCnjList,
      { ...Drafter.filterTemplate, val: juizoInfo.instanciaCnj },
    );
    projurisProcesso.orgaoJudicial = getProjurisItem(
      "orgaoJudicial",
      orgaosJudiciaisList,
      { ...Drafter.filterTemplate, val: juizoInfo.orgaoJudicial },
    );
    projurisProcesso.complementoVara = juizoInfo.comarca;
    const gts = this.#projurisHandler.filterProjurisOptions(
      this.#projurisLists.gtsList,
      { ...Drafter.filterTemplate, val: clientRole?.gt },
    );
    projurisProcesso.gruposDeTrabalho = gts ? gts[0] : undefined;
    try {
      const gtCrew = await getGtCrew(
        clientRole?.gt,
        this.#projurisLists.allResponsaveisList,
        this.#googleToken,
      );
    ...
    projurisProcesso.classeCnj = this.#fetchCnjItem(
      this.#processoInfo.tipoDeAcao,
      "classe",
    );
    projurisProcesso.assuntoCnj = this.#fetchCnjItem(
      this.#processoInfo.causaDePedir,
      "assunto",
    );
  ...
  #makeProjurisProcessoFetches(codTipoJustica, comarca) {
    const endPointInstanciaCnj =
      this.#projurisHandler.endPoints.instanciasCnj + codTipoJustica;
    const maxProjurisEntriesPerPage = 30;
    const endPointOrgaoJudicial =
      this.#projurisHandler.endPoints.orgaoJudicial +
      "(nomeOrgao:" +
      comarca +
      "$processoJusticaCodigo:" +
      codTipoJustica +
      "$pagina:0$quantidadeRegistros:" +
      maxProjurisEntriesPerPage +
      ")";
  ...
  #fetchCnjItem(itemPath, itemType) {
    ...
    const foundItem = this.#recursivelyGetMenuItem(
      itemsUniverse,
      structuredClone(itemPath),
    );
  ...
  #recursivelyGetMenuItem(menuItemsArray, itemPath) {
    if (!itemPath) return;
    const searchTermsPath = structuredClone(
      Array.isArray(itemPath) ? itemPath : [itemPath],
    );
    const firstItemInPath = searchTermsPath[0];
    const foundItem = menuItemsArray.filter(
      (menuItem) =>
        menuItem.chave === firstItemInPath.id ||
        menuItem.valor.toLowerCase() === firstItemInPath.valor.toLowerCase(),
    );
    if (foundItem.length > 0)
      return this.#handleFoundSomething(foundItem[0], searchTermsPath);
  ...
  #flatAndSeekAgain(menuItemsArray, searchTermsPath) {
    ...
    if (oneLevelChildrenArray.length === 0) return [];
      ...
      return this.#recursivelyGetMenuItem(
        oneLevelChildrenArray,
        searchTermsPath,
      );
  }

  #getCamposDinamicosProjuris() {
    const numOrgaoSegundoGrau = {
      campoDinamicoTipo: "LISTA_SELECAO_UNICA",
      codigoCampoDinamico: 1186,
    };
    const tipoOrgaoSegundoGrau = {
      campoDinamicoTipo: "LISTA_SELECAO_UNICA",
      codigoCampoDinamico: 1185,
    };
    const relatorSegundoGrau = {
      campoDinamicoTipo: "LISTA_MULTIPLA_SELECAO",
      codigoCampoDinamico: 1189,
    };
    const numOrgaoTerceiroGrau = {
      campoDinamicoTipo: "LISTA_SELECAO_UNICA",
      codigoCampoDinamico: 1187,
    };
    const tipoOrgaoTerceiroGrau = {
      campoDinamicoTipo: "LISTA_SELECAO_UNICA",
      codigoCampoDinamico: 1188,
    };
    const faturamento = this.#getFaturamentoProjuris();
    const sistema = this.#getSistemaProjuris();
    const valorInicialLiquidacao = {
      campoDinamicoTipo: "TEXTO_LONGO",
      codigoCampoDinamico: 4330,
    };
    const temasProcessuais = {
      campoDinamicoTipo: "LISTA_MULTIPLA_SELECAO",
      codigoCampoDinamico: 4689,
    };
    return [
      numOrgaoSegundoGrau,
      tipoOrgaoSegundoGrau,
      relatorSegundoGrau,
      numOrgaoTerceiroGrau,
      tipoOrgaoTerceiroGrau,
      faturamento,
      sistema,
      valorInicialLiquidacao,
      temasProcessuais,
    ];
  }
  ...
  #getFaturamentoProjuris() {
    ...
    const faturamentoProjurisInfo = this.#getItemsInfo(
      faturamentosInfo,
      "Não faturado",
    );
    return {
  ...
  #getSistemaProjuris() {
    ...
    const sistemaProjurisInfo = this.#getItemsInfo(
      sistemasInfo,
      Drafter.#nomeSistemaProjuris[this.#processoInfo.sistema.toLowerCase()],
    );
    return {
      campoDinamicoTipo: "LISTA_SELECAO_UNICA",
  ...
  #getItemsList(codigoCampoDinamico) {
    const itemsInfo = this.#projurisLists.camposDinamicosList.filter(
      (campoDinamico) => campoDinamico.codigo === codigoCampoDinamico,
    );
  ...
  #getItemsInfo(itemsList, itemName) {
    ...
    const matches = itemsList.filter(
  ...
  async #getAdaptedAndamentos() {
    ...
    projurisAndamentos.values = relevantAndamentos.map((andamento) =>
      this.#adaptAndamentoToProjuris(
        andamento,
        tiposAndamento,
        projurisAndamentos,
      ),
  ...
  #filterRelevantAndamentos() {
    return this.#processoInfo.andamentos.filter(
      (andamento) => andamento.nomeAdaptadoAoCliente ?? false,
    );
  }

  async #getAllTiposAndamento() {
    const maxProjurisEntriesPerPage = 100;
    const endPointTipoAndamento =
      this.#projurisHandler.endPoints.tiposAndamento +
      "quan-registros=" +
      maxProjurisEntriesPerPage;
    const response = await this.#projurisHandler.fetchProjurisInfo(
      endPointTipoAndamento,
    );
  ...
  #adaptAndamentoToProjuris(andamento, tiposAndamento, projurisAndamentos) {
    ...
    const tipoAndamento = tiposAndamento.filter(
      (tipoAndamento) =>
        tipoAndamento.nome.toLowerCase() ===
        andamento.nomeAdaptadoAoCliente.toLowerCase(),
    );
    if (Array.isArray(tipoAndamento) && tipoAndamento.length === 0) {
      projurisAndamentos.errorMsgs.push(
        generateErrMsg.noMatchInGoogle(
          andamento.nomeAdaptadoAoCliente,
          "andamento",
        ),
      );
      return this.#errorMsgFallback;
    } else {
      projurisAndamento.codigoTipoAndamento =
        tipoAndamento[0].codigoAndamentoTipo;
    }
  ...
  async #getAdaptedPedidos(clientName, dataDistribuicao, causasDePedir = []) {
    ...
    if (clientName) {
      projurisPedidos.values =
        await this.#getStandardPedidosByClientAndCausaPedir(
          clientName,
          dataDistribuicao,
          causasDePedir,
        );
      const pedidosList = await this.#fetchRelevantPedidosProjurisData(
        projurisPedidos.values,
      );
  ...
  async #getStandardPedidosByClientAndCausaPedir(
    clientName,
    dataDistribuicao,
    causasDePedir = [],
  ) {
    const clientFilteredProvisionsList =
      this.#projurisHandler.filterProjurisOptions(
        this.#sheetsLists.allClientsProvisionsList,
        {
          key: 0,
          operator: operators.insensitiveStrictEquality,
          val: clientName,
    ...
    const clientProvisionsByCausasDePedir =
      this.#filterClientsProvisionsByCausasDePedir(
        clientFilteredProvisionsList,
        causasDePedir,
      );
  ...
  async #fetchRelevantPedidosProjurisData(pedidos) {
    const promises = pedidos.map((pedido) =>
      this.#projurisHandler.fetchProjurisInfo(
        this.#projurisHandler.endPoints.pedidos + pedido.nomePedido,
      ),
    );
    ...
    return await Promise.all(
      responses.map(
        async (response) =>
          await this.#projurisHandler.extractOptionsArray(response),
      ),
    );
  }
  ...
  #pushNomeAndCodigoIntoPedidos(projurisPedidos, list) {
    projurisPedidos.values.forEach((pedido, index) => {
      ...
      if (relatedTypes === "no content") {
        projurisPedidos.errorMsgs.push(
          generateErrMsg.noMatchInProjuris(pedido.nomePedido, "pedido"),
        );
        return;
      }
      const type = relatedTypes.filter(
        (type) => type.valor === pedido.nomePedido,
      );
  ...
  #filterClientsProvisionsByCausasDePedir(provisionsList, causasDePedir) {
    if (causasDePedir.length === 0) {
      ...
      return provisionsList.filter(
        (clientProvision) =>
          clientProvision[1].trim().toLowerCase() === fallbackValue,
      );
    } else {
      return provisionsList.filter((clientProvision) =>
        causasDePedir.includes(clientProvision[1]),
      );
    }
  }
  ...
  async #getAdaptedFaturamentos(clientName) {
    ...
    if (clientName) {
      projurisFaturamentos.values =
        await this.#getStandardFaturamentosByClient(clientName);
    }
    return projurisFaturamentos;
  }

  async #getStandardFaturamentosByClient(clientName) {
    const clientFilteredFaturamentosList =
      this.#projurisHandler.filterProjurisOptions(
        this.#sheetsLists.allClientsFaturamentosList,
        {
          key: 0,
          operator: operators.insensitiveStrictEquality,
          val: clientName,
    ...
    const clientFaturamentosAfterConditions =
      this.#filterClientsFaturamentosByConditions(
        clientFilteredFaturamentosList,
      );
    return clientFaturamentosAfterConditions.map((faturamento) => {
      const [
        ...
        condicao,
        valorRawStr,
        vencimentoCalculationUnitsRawStr,
        unitsToVencimentoRawStr,
        bancoRawStr,
      ] = faturamento;
      ...
      const banco = this.#projurisHandler.filterProjurisOptions(
        this.#projurisLists.bancosFormattedList,
        {
          key: "valor",
          operator: operators.insensitiveStrictEquality,
          val: bancoRawStr,
      ...
      return new ProjurisFaturamentoDataStructure(
        clientName,
        valor,
        new Date(),
        vencimento,
  ...
  #calculateVencimento(calculationUnit, unitsToVencimento, condicao) {
    const baseDate =
      condicao.toLowerCase() === "audiência"
        ? new Date(this.#processoInfo.audienciaFutura.data)
        : new Date();
    const vencimentoCalc = {
      ...
      dias: (days) =>
        new Date(baseDate.getTime() + parseInt(days) * 24 * 60 * 60 * 1000),
    };
