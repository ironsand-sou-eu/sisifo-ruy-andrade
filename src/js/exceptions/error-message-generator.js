...
const generateErrMsg = {
  noMatchInGoogle: (missingEntry, type) => {
    return (
      `Não encontramos ${textualReferenceType[type]} correspondente a ` +
  ...
  noMatchInProjuris: (missingEntry, type) => {
    return (
      ...
      `informações no Projuris e na ${linkOrText(
        type,
...
const generateValidationMsg = {
  generalWarning: () => {
    return (
      "Foram encontrados as situações abaixo. Se você quiser cadastrar o processo " +
  ...
  noClient: () => {
    return (
      "Você não indicou qual das partes é o cliente. Para fazê-lo, vá " +
  ...
  noClientNumber: () => {
    return (
      "Você não indicou um número de pasta no sistema do cliente (isso só faz " +
  ...
  noWorkgroup: () => {
    return (
      "Você não indicou o grupo de trabalho responsável pelo processo. Isso pode " +
  ...
  noLawyer: () => {
    return (
      "Você não indicou o(s) advogado(s) responsável(is) pelo processo. Isso pode " +
      "fazer com que as tarefas não sejam cadastradas corretamente."
    );
  },
};
