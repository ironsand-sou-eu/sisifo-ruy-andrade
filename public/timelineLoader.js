async function loadEntireTimeline(fullLoad) {
  ...
  const pagesTotal = parseInt(
    document.querySelector("#totalPaginas").getAttribute("value"),
  );
  const currentPage = parseInt(
    document.querySelector("#paginaAtual").getAttribute("value"),
  );
  ...
  if (pagesTotal > autoloadPagesMaxQuantity) {
    fullLoad =
      fullLoad ??
      confirm(
        `O processo tem ${pagesTotal} páginas. Deseja ` +
